...
from ..crud.donut import create_donut, get_donut, update_donut, delete_donut
...
async def create_donut_endpoint(donut: DonutCreate, db: AsyncSession = Depends(db_helper.session_dependency)):
    return await create_donut(db, donut)


@router.get("/{donut_id}", response_model=DonutRead)
async def read_donut(donut_id: int, db: AsyncSession = Depends(db_helper.session_dependency)):
    return await get_donut(db, donut_id)


@router.put("/{donut_id}", response_model=DonutRead)
async def update_donut_endpoint(donut_id: int, donut: DonutUpdate,
...
async def upload_image(donut_id: int, file: UploadFile = File(...),
                       db: AsyncSession = Depends(db_helper.session_dependency)):
...
async def get_image(donut_id: int, db: AsyncSession = Depends(db_helper.session_dependency)):
    db_donut = await get_donut(db, donut_id)
